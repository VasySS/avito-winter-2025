...
func (h *Argon2PasswordHasher) GenerateFromPassword(password []byte, _ int) ([]byte, error) {
...
func (h *Argon2PasswordHasher) CompareHashAndPassword(hashedPassword, password []byte) error {
	...
	if subtle.ConstantTimeCompare(hash, otherHash) != 1 {
		return fmt.Errorf("failed to compare hash and password using argon2")
	}
...
func decodeHash(encodedHash string) (p *params, salt, hash []byte, err error) {
	...
	if len(vals) != 6 {
		return nil, nil, nil, fmt.Errorf("invalid encoded hash: %s", encodedHash)
	}
